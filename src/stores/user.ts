...
export const useUser = defineStore("user", () => {
    ...
    async function deleteUser(id: string) {
        ...
        if (totalUsers <= 1) {
            try {
                ...
                throw new Error("Cannot delete only user");
            }
        }
    ...
    async function changeUser(id: number) {
        if (!currentUser.value || currentUser.value.id === id) {
            throw new Error("Invalid action during user change");
        }
        const tx = idb.transaction("users", "readwrite");
        const store = tx.objectStore("users");
        const newUser = await store.get(IDBKeyRange.only(id));
        const curUser = await store.get(IDBKeyRange.only(currentUser.value.id));
